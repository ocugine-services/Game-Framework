...
//  @module Ocugine2D
//=======================================================================
var ocugine2D = function(OcugineGF) {
  "use strict";

  OcugineGF["2D"] = function(GF) {
    // Viewport Component
    GF.component('viewport',{
      added: function() {
        ...
        this.offsetX = 0;
        this.offsetY = 0;
        this.centerX = GF.width/2;
        this.centerY = GF.height/2;
        this.scale = 1;
      },
      ...
      extend: {
        follow: function(sprite,directions,boundingBox) {
          ...
          if(GF._isUndefined(boundingBox) && this.lists.TileLayer !== undefined) {
            this.viewport.boundingBox = GF._detect(this.lists.TileLayer, function(layer) {
      ...
      follow: function(first) {
        var followX = GF._isFunction(this.directions.x) ? this.directions.x(this.following) : this.directions.x;
        var followY = GF._isFunction(this.directions.y) ? this.directions.y(this.following) : this.directions.y;
      ...
      softCenterOn: function(x,y) {
        if(x !== void 0) {
          var dx = (x - GF.width / 2 / this.scale - this.x)/3;
          if(this.boundingBox) {
            if(this.x + dx < this.boundingBox.minX) {
              this.x = this.boundingBox.minX / this.scale;
            }
            else if(this.x + dx > (this.boundingBox.maxX - GF.width) / this.scale) {
              this.x = Math.max(this.boundingBox.maxX - GF.width, this.boundingBox.minX) / this.scale;
            }
            else {
              this.x += dx;
            }
        ...
        if(y !== void 0) {
          var dy = (y - GF.height / 2 / this.scale - this.y)/3;
          if(this.boundingBox) {
            if(this.y + dy < this.boundingBox.minY) {
              this.y = this.boundingBox.minY / this.scale;
            }
            else if(this.y + dy > (this.boundingBox.maxY - GF.height) / this.scale) {
              this.y = Math.max(this.boundingBox.maxY - GF.height, this.boundingBox.minY) / this.scale;
            }
            else {
              this.y += dy;
            }
      ...
      centerOn: function(x,y) {
        if(x !== void 0) {
          this.x = x - GF.width / 2 / this.scale;
        }
        if(y !== void 0) {
          this.y = y - GF.height / 2 / this.scale;
        }
      ...
      prerender: function() {
        this.centerX = this.x + GF.width / 2 /this.scale;
        this.centerY = this.y + GF.height / 2 /this.scale;
        GF.ctx.save();
        GF.ctx.translate(Math.floor(GF.width/2),Math.floor(GF.height/2));
        GF.ctx.scale(this.scale,this.scale);
        GF.ctx.translate(-Math.floor(this.centerX), -Math.floor(this.centerY));
      },
    ...
    GF.Sprite.extend("TileLayer",{
      ...
      _generateCollisionObjects: function() {
        ...
        if(this.sheet() && this.sheet().frameProperties) {
          ...
          for(var k in frameProperties) {
            var colObj = this.tileCollisionObjects[k] = { p: GF._clone(this.collisionObject.p) };
            GF._extend(colObj.p,frameProperties[k]);

            if(colObj.p.points) {
              colObj.p.points = GF._map(colObj.p.points, returnPoint);
            }
      ...
      load: function(dataAsset) {
        ...
        if (fileExt === "json") {
          data = GF._isString(dataAsset) ?  GF.asset(dataAsset) : dataAsset;
        }
        else {
    ...
    GF.component('2d',{
      added: function() {
        ...
        GF._defaults(entity.p,{
          vx: 0,
          vy: 0,
          ax: 0,
          ay: 0,
          gravity: 1,
          collisionMask: GF.SPRITE_DEFAULT
        });
      ...
      step: function(dt) {
        ...
        while(dtStep > 0) {
          ...
          p.vx += p.ax * dt + (p.gravityX === void 0 ? GF.gravityX : p.gravityX) * dt * p.gravity;
          p.vy += p.ay * dt + (p.gravityY === void 0 ? GF.gravityY : p.gravityY) * dt * p.gravity;
          p.x += p.vx * dt;
          p.y += p.vy * dt;
